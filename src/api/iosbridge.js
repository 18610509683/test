...
(function(mdSmartios) {
    ...
    /**
     * ^6.3转换在线base64图片成（H5自行转换有跨域问题）
     * @param {json} param  参数
     * {
        url: xxxx, //url为http开头的在线图片或file开头的手机本地图片
        }
     * @return {json} 処理結果
        {
        base64: "xxxxxxxx"
    ...
    mdSmartios.bridge.convertImageToBase64 = function (param, callback, callbackFail) {
        ...
        var commandId = mdSmartios.bridge.po._execObjcMethod('convertImageToBase64', p);

        return commandId;
    };
    /**
     * ^6.4转换本地base64图片成（H5自行转换有跨域问题）
     * @param {json} param  参数
     * {
        url: xxxx, //url为http开头的本地图片或file开头的手机本地图片
        }
     * @return {json} 処理結果
        {
        base64: "xxxxxxxx"
    ...
    mdSmartios.bridge.convertLocalImageToBase64 = function (param, callback, callbackFail) {
        ...
        var commandId = mdSmartios.bridge.po._execObjcMethod('convertLocalImageToBase64', p);
    ...
    mdSmartios.bridge.killkeyboard = function () {
      ...
      mdSmartios.bridge.po._execObjcMethod('killkeyboard');
    };
    ...
    mdSmartios.bridge.openNativeSystemSetting = function (settingName) {
      ...
      var param = {};
      param.setting = settingName || 'bluetooth';
      let p = JSON.stringify(param);
      mdSmartios.bridge.po._execObjcMethod('openNativeSystemSetting', p);
    };

    /**
      *^6.4拍照功能
      *param{Number} compressRage 返回照片的压缩率，范围为0~100，数值越高保真率越高
      *param{Sting} type 值为jpg或png，指定返回相片的格式
      *param{Boolean} isNeedBase64 是否需要返回相片base64数据
      */
   mdSmartios.bridge.takePhoto = function(param, callback, callbackFail) {
    console.log('function:mdSmartios.bridge.takePhoto: callback=' + callback);
    if (param == undefined || param == '') {
      param = {};
    }
    param.cammandId = Math.floor(Math.random() * 100000);
    ...
    var commandId = mdSmartios.bridge.po._execObjcMethod('takePhoto', p);

    return commandId;
    };
   /**
      *^6.4拍照功能
      *param{string} path 值为 图片在手机中的路径
      *param{string} url 值为服务器上传图片的url
      *param{number} maxWidth 最大宽度，如果不设置，则使用图片宽度
      *param{number} maxHeight 最大高度，如果不设置，则使用图片高度
      *param{number} compressRage 图片的压缩率，范围为0~100，数值越高保真率越高。默认值：100，不压缩，直接上传图片 ps: 压缩后的图片文件格式，固定为jpg 格式
      *param{Object} netParam weex需要原生填充给服务器的post 表单参数1
      *param{string} fileKey 值为原生在post表单中传输图片文件的key值，缺省默认值为“file”
      */
     mdSmartios.bridge.uploadImgFileToMas = function(param, callback, callbackFail) {
      ...
      var commandId = mdSmartios.bridge.po._execObjcMethod('uploadImgFileToMas', p);

      return commandId;
    };

    /**
      *^6.4选择多张相册照片，并返回相片数据，不支持base64的转换
      *max{Number} max 一次最多可选择的数量，默认为9，最多9张
      */
    mdSmartios.bridge.chooseMulPhoto = function(param, callback, callbackFail) {
        console.log('function:mdSmartios.bridge.chooseMulPhoto: callback=' + callback);
        if (param == undefined || param == '') {
          param = {};
        }
        ...
        var commandId = mdSmartios.bridge.po._execObjcMethod('chooseMulPhoto', p);
        return commandId;
    };
...
//        }
    ...
    mdSmartios.bridge.setApplianceConnnectStatus = function(pIsConnect) {
        ...
        if (mdSmartios.bridge.isApplianceConnected == 0x01) {
    ...
    // GPS信息获取 JS－>iOS
    mdSmartios.bridge.getGPSInfo = function(param, callback, callbackFail) {
    ...
    mdSmartios.bridge.getUserInfoBase64 = function(callback) {
        ...
        var commandId = mdSmartios.bridge.po._execObjcMethod('getUserInfoBase64', p);
        return commandId;
    };
...
//        }
    ...
    */
     ...
     * http://confluence.msmart.com/pages/viewpage.action?pageId=21498414
     */
    mdSmartios.bridge.goToMeijuPageWithUrl = function (url) {
        console.log('function:mdSmartios.bridge.goToMeijuPageWithUrl');
        var p = JSON.stringify({url: url});
        var commandId = mdSmartios.bridge.po._execObjcMethod('goToMeijuPageWithUrl', p);
        return commandId;
    };
